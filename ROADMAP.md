...
- [x] 트랜잭션 격리 수준(Isolation Level)이란 (Dirty/Non-Repeatable/Phantom Read, Lost Update, MySQL 기본값) → [레슨](lessons/0044-transaction-isolation-level.html) | [배운 작업](work-log/2026-08-11-ppback-pr-5532-lost-update-lock-review.md) · [배운 작업](work-log/2026-08-12-key-result-auto-checkin-reflect-lock-snapshot-verification.md) · MySQL REPEATABLE READ의 스냅샷 고정 **시점** 규칙: 잠금 읽기(`FOR UPDATE`)는 read view를 열지 않고 **첫 비잠금 SELECT가 연다.** 그래서 "락을 트랜잭션이 열리기 전(또는 첫 문장)에 잡아야 한다"는 제약이 생긴다 · 실측 확인: `FOR UPDATE`가 최신 커밋값을 보여줘도 트랜잭션의 스냅샷 자체는 갱신되지 않아, 그 직후 평범한 SELECT는 다시 옛 스냅샷으로 돌아간다
