...
>
> ⚠️ **내용 출처가 다르다.** `net-*` 은 강의를 들으며 정리한 것이지만, `java-*`·`spring-*` 은 **강의를 수강하지 않고 만든다.**
> 대신 아래 절차로 강의 내용을 간접적으로 확보한다.
>
> **레슨 제작 절차 (필수)**
>
> 1. 해당 섹션 제목으로 웹 검색해 **수강생 정리 글 3편 이상**을 찾는다
>    (검색은 `/browse` 스킬. 구글은 헤드리스에 CAPTCHA를 걸므로 **네이버 검색이 가장 잘 먹는다**.
>    velog·tistory·github.io 글이 대체로 상세하다).
> 2. 3편을 **교차 확인**해서 강의의 실제 전개·예제·용어를 파악한다. 한 편에만 나오는 내용은 신뢰하지 않는다.
> 3. 그 전개를 뼈대로 삼되, **Ruby·TypeScript 대비와 뿌 연결은 직접 쓴다.** 블로그 문장을 그대로 옮기지 않는다.
> 4. 레슨 상단 `mission-link` 에 "강의 미수강 · 정리 글 N편 교차 확인" 을 명시하고,
>    하단 `source` 에 **참고한 글 전부를 링크**한다. 직접 쓴 부분도 구분해 적는다.
>
> 이 절차를 거치면 강의를 안 듣고도 "무엇을 다루는지"와 "어떤 예제로 설명하는지"까지는 맞출 수 있다.
> 다만 강의 원문이 아니므로, 나중에 강의를 듣게 되면 세부 전개가 다를 수 있다는 것을 알고 봐야 한다.
...
- [x] `java-10` 클래스와 데이터 → [레슨](lessons/java-10-class-and-data.html) · 클래스·객체·인스턴스 세 단어 구분과 참조변수 aliasing 추가(2026-09-22)
- [ ] `java-11` 기본형과 참조형
- [ ] `java-12` 객체 지향 프로그래밍
- [ ] `java-13` 생성자
- [ ] `java-14` 패키지
